...
type CredentialKey =
  ...
  | 'refreshToken'
  | 'redirectUri'
export type Credentials = Partial<Record<CredentialKey, string | null>>
...
interface GetArtistAlbumsOptions extends PaginationCountryOptions {
  include_groups?: string | IncludeGroup | IncludeGroup[] | undefined
}
...
export default class SpotifyWebApi {
  ...
  setAccessToken(accessToken: string) {
    this.setCredential('accessToken', accessToken)
  }

  setRefreshToken(refreshToken: string) {
    this.setCredential('refreshToken', refreshToken)
  }

  setRedirectURI(redirectUri: string) {
    this.setCredential('redirectUri', redirectUri)
  }

  getRedirectURI() {
    return this.getCredential('redirectUri')
  }
  ...
  getRefreshToken() {
    return this.getCredential('refreshToken')
  }

  resetClientId() {
    this.resetCredential('clientId')
  }

  resetClientSecret() {
    this.resetCredential('clientSecret')
  }

  resetAccessToken() {
    this.resetCredential('accessToken')
  }

  resetRefreshToken() {
    this.resetCredential('refreshToken')
  }

  resetRedirectURI() {
    this.resetCredential('redirectUri')
  }

  private setCredential(credentialKey: CredentialKey, value: string) {
    this.credentials = this.credentials || {}
    this.credentials[credentialKey] = value
  }

  private getCredential(credentialKey: CredentialKey) {
    if (!this.credentials) {
      return null
    } else {
  ...
  private resetCredential(credentialKey: CredentialKey) {
    if (!this.credentials) {
      return
    } else {
  ...
  getUser(userId: string, callback?: Callback<SpotifyApi.UserProfileResponse>) {
    return WebApiRequest.builder(this.getAccessToken())
  ...
  getMe(callback?: Callback<SpotifyApi.CurrentUsersProfileResponse>) {
    return WebApiRequest.builder(this.getAccessToken())
      .withPath('/v1/me')
      .build()
      .execute(HttpManager.get, callback)
  ...
  getUserPlaylists(
    userId?: string | PaginationOptions,
    options?:
      | PaginationOptions
      | Callback<SpotifyApi.ListOfUsersPlaylistsResponse>,
    callback?: Callback<SpotifyApi.ListOfUsersPlaylistsResponse>,
  ) {
    let path = '/v1/me/playlists'
  ...
  followPlaylist(
    playlistId: string,
    options?: PublicOptions,
  ): Promise<Response<SpotifyApi.FollowPlaylistResponse>>
  followPlaylist(
    playlistId: string,
    options: PublicOptions,
    callback?: Callback<SpotifyApi.FollowPlaylistResponse>,
  ...
  uploadCustomPlaylistCoverImage(
    playlistId: string,
    base64URI: string,
  ): Promise<Response<SpotifyApi.UploadCustomPlaylistCoverImageResponse>>
  uploadCustomPlaylistCoverImage(
    playlistId: string,
    base64URI: string,
    callback?: Callback<SpotifyApi.UploadCustomPlaylistCoverImageResponse>,
  ...
  getMyDevices(callback?: Callback<SpotifyApi.UserDevicesResponse>) {
    return WebApiRequest.builder(this.getAccessToken())
  ...
  play(options?: PlayOptions, callback?: Callback<void>) {
  ...
  pause(options?: DeviceOptions, callback?: Callback<void>) {
    return (
  ...
  skipToPrevious(options?: DeviceOptions, callback?: Callback<void>) {
    return WebApiRequest.builder(this.getAccessToken())
      .withPath('/v1/me/player/previous')
      .withQueryParameters(
        options?.device_id ? { device_id: options.device_id } : null,
      )
      .build()
      .execute(HttpManager.post, callback)
  }
  ...
  skipToNext(options?: DeviceOptions, callback?: Callback<void>) {
    return WebApiRequest.builder(this.getAccessToken())
  ...
  seek(positionMs: number, options?: DeviceOptions, callback?: Callback<void>) {
    var params = {
      /* jshint camelcase: false */
  ...
  followUsers(userIds: ReadonlyArray<string>, callback?: Callback<void>) {
    return WebApiRequest.builder(this.getAccessToken())
  ...
  followArtists(artistIds: ReadonlyArray<string>, callback?: Callback<void>) {
    return WebApiRequest.builder(this.getAccessToken())
      .withPath('/v1/me/following')
      .withQueryParameters({
        ids: artistIds.join(','),
  ...
  unfollowUsers(userIds: ReadonlyArray<string>, callback?: Callback<void>) {
    return WebApiRequest.builder(this.getAccessToken())
      .withPath('/v1/me/following')
      .withQueryParameters({
        ids: userIds.join(','),
  ...
  unfollowArtists(artistIds: ReadonlyArray<string>, callback?: Callback<void>) {
    return WebApiRequest.builder(this.getAccessToken())
      .withPath('/v1/me/following')
      .withQueryParameters({
        ids: artistIds.join(','),
  ...
  areFollowingPlaylist(
    ...
    return WebApiRequest.builder(this.getAccessToken())
      .withPath(
        `/v1/users/${userId}/playlists/${playlistId}/followers/contains`,
      )
      .withQueryParameters({
