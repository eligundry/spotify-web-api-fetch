import AuthenticationRequest from '../src/authentication-request'
...
describe('Create Authentication Requests', () => {
  test('Should use default settings if none are supplied', () => {
    ...
    expect(request.getHeaders()).toEqual(new Headers())
    expect(request.getPath()).toBeFalsy()
    expect(request.getQueryParameters()).toEqual(new URLSearchParams())
